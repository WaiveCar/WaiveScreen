...
selfie() {
  ...
  echo $(curl -X POST -F "f0=$cache/$now-screen.jpg" -F "f1=$cache/$now-0.jpg" -F "f2=$cache/$now-2.jpg" -F "f3=$cache/$now-4.jpg" -F "f4=$cache/$now-6.jpg" "waivescreen.com/selfie?pre=$now")
}
